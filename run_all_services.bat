...
:: 1.1 Config Server
echo [2/10] Starting CONFIG SERVER (Port 8888)...
...
:: 2. Core Services
echo [3/10] Starting USER SERVICE (Port 8050)...
start "User Service" cmd /k "cd user-service && mvn spring-boot:run"

echo [4/10] Starting PRODUCT SERVICE (Port 8051)...
start "Product Service" cmd /k "cd product-service && mvn spring-boot:run"

echo [5/10] Starting INVENTORY SERVICE (Port 8082)...
...
echo [9/10] Starting PAYMENT SERVICE (Port 8054)...
start "Payment Service" cmd /k "cd payment-service && mvn spring-boot:run"

echo [10/10] Starting SHIPPING SERVICE (Port 8055)...
